...
if (COUNT_TO_BASE in (toLowerANSI _menuName)) exitWith {
	...
	// get the number at the end of the menu name string to denote the count
...
if (CHECK_MENU_NAME(CAS_TYPES_ALL_MENU)) exitWith {
	...
	[
		...
		STD_LINE("Bomb (Laserguided)",8,PUSHBACK_AND_PROCEED(BOMB_LGB_ID)),
...
if (CHECK_MENU_NAME(CAS_TYPES_BOMBS_MENU)) exitWith {
	...
	[
		...
		STD_LINE("Bomb (Laserguided)",3,PUSHBACK_AND_PROCEED(BOMB_LGB_ID)),
